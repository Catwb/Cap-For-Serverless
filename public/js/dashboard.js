...
const formatRelative = (date) => {
  ...
  for (const [u, v] of Object.entries(ms)) {
    if (d >= v) {
      ...
      return past ? _t("n_unit_ago", val, _tp(u, val)) : _t("in_n_unit", val, _tp(u, val));
    }
  }
  return past ? _t("just_now") : _t("in_a_moment");
};
...
async function loadKeys() {
  ...
  if (keys?.error) {
    keysList.innerHTML = '<div class="keys-empty"><p>' + _t("error_loading_keys") + '</p></div>';
    return;
  }
  renderKeysList();
}
  ...
  if (filtered.length === 0) {
    keysList.innerHTML = `
      <div class="keys-empty">
        <p>${filter ? _t("no_matching_keys") : _t("no_keys_yet")}</p>
      </div>`;
    return;
  }
  keysList.innerHTML = filtered
    ...
    <div class="key-item ${selectedKey?.siteKey === key.siteKey ? "active" : ""}" data-key="${key.siteKey}">
      ...
      <div class="key-item-stats ${key.difference?.direction === "up" ? "trend-up" : key.difference?.direction === "down" ? "trend-down" : "trend-neutral"}">
        ...
        ${_t("n_recent_solves", formatCompact(key.solvesLast24h || 0))}
      </div>
    </div>`,
    )
    .join("");
...
3. Instruct the user to open an issue on GitHub: https://github.com/tiagozip/cap`;
  return `
    <div class="integration-layout">
      <h3 class="config-section-title">${_t("frontend")}</h3>
      <p class="integration-hint">${_t("docs_hint")}</p>
      <div class="code-block" data-raw="${escapeHtml(widget)}">
        <button class="code-copy">${_t("copy")}</button>
        <pre><code class="hl">${highlight(widget)}</code></pre>
      </div>

      <h3 class="config-section-title" style="margin-top: 24px;">${_t("server_verification")}</h3>
      ...
      <h3 class="config-section-title" style="margin-top: 24px;">${_t("ai_prompt")}</h3>
      <p class="integration-hint">${_t("ai_hint")}</p>
      <div class="code-block code-block-prompt" data-raw="${escapeHtml(aiPrompt)}">
        ...
        <button class="code-copy code-copy-large">
          ...
          <span class="code-copy-label">${_t("copy_prompt")}</span>
        </button>
      </div>
    </div>
  `;
  ...
  root.querySelectorAll(".code-copy").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      navigator.clipboard.writeText(text).then(() => {
        ...
        label.textContent = _t("copied_tooltip");
...
function renderKeyDetail() {
  ...
  keyDetail.innerHTML = `
        ...
        <div class="activity-main">
          <div class="filters-row">
            <select class="time-select" id="timeSelect">
              <option value="today" ${key.chartData?.duration === "today" ? "selected" : ""}>${_t("today")}</option>
              <option value="yesterday" ${key.chartData?.duration === "yesterday" ? "selected" : ""}>${_t("yesterday")}</option>
              <option value="last7days" ${key.chartData?.duration === "last7days" ? "selected" : ""}>${_t("last_7_days")}</option>
              <option value="last28days" ${key.chartData?.duration === "last28days" ? "selected" : ""}>${_t("last_30_days")}</option>
              <option value="last91days" ${key.chartData?.duration === "last91days" ? "selected" : ""}>${_t("last_3_months")}</option>
              <option value="alltime" ${key.chartData?.duration === "alltime" ? "selected" : ""}>${_t("all_time")}</option>
            </select>
          ...
          <div class="stats-row">
            <div class="stat-item">
              ...
              <div class="stat-label">${_t("stat_challenges")}</div>
            ...
            <div class="stat-item">
              ...
              <div class="stat-label">${_t("stat_verified")}</div>
            ...
            <div class="stat-item">
              ...
              <div class="stat-label">${_t("stat_failed")}</div>
            ...
            <div class="stat-item">
              ...
              <div class="stat-label">${_t("stat_avg_duration")}</div>
          ...
          <div class="insights-grid" id="insightsGrid">
            <div class="insight-panel" id="locationPanel">
              <div class="insight-panel-header">
                <h3 class="insight-panel-title">${_t("location")}</h3>
                <div class="insight-view-toggle" id="locationViewToggle">
                  <button class="insight-toggle-btn ${!locationMapMode ? "active" : ""}" data-view="list" title="${_t("list_view")}">
                  ...
                  <button class="insight-toggle-btn ${locationMapMode ? "active" : ""}" data-view="map" title="${_t("map_view")}">
            ...
            <div class="insight-panel" id="networksPanel">
              <div class="insight-panel-header">
                <h3 class="insight-panel-title">${_t("networks")}</h3>
                <button class="insight-search-btn" id="networksSearchBtn" title="${_t("search_networks")}">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
                </button>
              </div>
              <div class="insight-search-bar" id="networksSearchBar" style="display:none">
                <input type="text" id="networksSearchInput" placeholder="${_t("filter_networks")}">
              </div>
            ...
            <div class="insight-panel" id="platformPanel">
              <div class="insight-panel-header">
                <h3 class="insight-panel-title">${_t("platform")}</h3>
              </div>
            ...
            <div class="insight-panel" id="osPanel">
              <div class="insight-panel-header">
                <h3 class="insight-panel-title">${_t("os")}</h3>
              </div>
    ...
    <div class="tab-content ${currentTab === "configuration" ? "active" : ""}" id="configurationTab">
      <div class="config-panel">
        <h3 class="config-section-title">${_t("config_main")}</h3>
        <div class="config-card">
          <div class="edit-field">
            <label>${_t("config_name")}</label>
            <input type="text" id="cfgName" value="${escapeHtml(key.name)}">
          </div>
          <div class="edit-field" style="margin-top:8px">
            <label>${_t("config_protocol")}</label>
            <select id="cfgChallengeProtocol">
              <option value="sha256-pow" ${!key.config.rsw ? "selected" : ""}>${_t("config_sha256")}</option>
              <option value="rsw" ${key.config.rsw ? "selected" : ""}>${_t("config_rsw")}</option>
            </select>
          </div>
          <div class="edit-row" id="shaPowFields" style="display:${key.config.rsw ? "none" : "flex"}">
            <div class="edit-field">
              <label>${_t("config_difficulty")}</label>
              <input type="number" id="cfgDifficulty" value="${key.config.difficulty}" min="1" max="8">
            </div>
            <div class="edit-field">
              <label>${_t("config_challenge_count")}</label>
          ...
          <div class="config-row" id="rswTField" style="display:${key.config.rsw ? "flex" : "none"}">
            <div class="range-field" style="flex:1">
              <label>${_t("config_rsw_difficulty")} <span class="range-value" id="rswTHint">${(key.config?.rswT ?? 75000).toLocaleString()}</span></label>
              <span class="range-hint">${_t("config_rsw_hint")}</span>
              <input type="range" id="cfgRswT" min="10000" max="300000" step="5000" value="${key.config.rswT ?? 75000}">
            </div>
          </div>
          <h3 class="config-section-title" style="margin-top:16px">${_t("config_instrumentation")}</h3>
          <div class="switch-field">
            <label class="switch">
              <input type="checkbox" id="cfgInstrumentation" ${key.config.instrumentation ? "checked" : ""}>
              <span class="switch-track"></span>
            </label>
            <label for="cfgInstrumentation" class="switch-label">${_t("config_enable_instrumentation")}</label>
          </div>
          <div class="switch-field" id="blockAutomatedBrowsersField" style="display:${key.config.instrumentation ? "flex" : "none"}">
            ...
            <label for="cfgBlockAutomatedBrowsers" class="switch-label">
              ${_t("config_block_headless")}
              <span class="hint">${_t("config_block_headless_hint")}</span>
            </label>
          </div>
          <div class="config-row" id="obfuscationLevelField" style="display:${key.config.instrumentation ? "flex" : "none"}">
            <div class="range-field" style="flex:1">
              <label>${_t("config_obfuscation")} <span class="range-value" id="obfuscationLevelHint">${key.config.obfuscationLevel ?? 5}</span></label>
              <span class="range-hint">${_t("config_obfuscation_hint")}</span>
              <input type="range" id="cfgObfuscationLevel" min="1" max="10" value="${key.config.obfuscationLevel ?? 5}">
            </div>
          </div>
          <div class="config-save-row">
            <button class="save-btn" id="saveMainConfigBtn" disabled>${_t("save")}</button>
          </div>
        </div>

        <h3 class="config-section-title">${_t("security")}</h3>
        <div class="config-card">
          <h4 class="config-subsection-title">${_t("rate_limiting")}</h4>
          <p class="headers-description" style="margin:-4px 0 8px">${_t("rate_limit_desc")}${ratelimitSettings ? ` (${ratelimitSettings.max} reqs / ${ratelimitSettings.duration / 1000}s)` : ""}.</p>
          <div class="edit-row">
            <div class="edit-field">
              <label>${_t("max_requests")}</label>
              <input type="number" id="cfgRatelimitMax" value="${key.config.ratelimitMax ?? ""}" min="1" max="10000" placeholder="${ratelimitSettings?.max ?? 30}">
            </div>
            <div class="edit-field">
              <label>${_t("window_ms")}</label>
          ...
          <h4 class="config-subsection-title">${_t("cors")}</h4>
          <div class="switch-field">
            <label class="switch">
              <input type="checkbox" id="cfgCorsEnabled" ${key.config.corsOrigins?.length ? "checked" : ""}>
              <span class="switch-track"></span>
            </label>
            <label for="cfgCorsEnabled" class="switch-label">${_t("restrict_origins")}</label>
          </div>
          <div id="keyCorsPanel" style="display:${key.config.corsOrigins?.length ? "block" : "none"}">
            <p class="headers-description" style="margin:0 0 8px">${_t("origins_desc")}</p>
            <div id="keyCorsOriginsList" class="origin-list">
              ${(key.config.corsOrigins || []).map((o) => `<div class="origin-entry"><input type="text" class="key-cors-origin-input" value="${escapeHtml(o)}" placeholder="${_t("add_origin")}"><button class="origin-remove-btn" title="${_t("remove")}">&times;</button></div>`).join("")}
            </div>
          </div>

          <hr class="settings-divider">

          <h4 class="config-subsection-title">${_t("request_filtering")}</h4>
          <p class="headers-description" style="margin:-4px 0 8px">${_t("filtering_desc")}</p>
          <div class="switch-field">
            <label class="switch">
            ...
            <label for="cfgBlockNonBrowserUA" class="switch-label">
              ${_t("block_non_browser")}
              <span class="hint">${_t("block_non_browser_hint")}</span>
            </label>
          </div>
          <div class="switch-field">
            <label class="switch">
            ...
            <label for="cfgRequiredHeadersEnabled" class="switch-label">
              ${_t("require_browser_headers")}
              <span class="hint">${_t("require_browser_headers_hint")}</span>
            </label>
          </div>
          ...
          <div class="config-save-row">
            <button class="save-btn" id="saveSecurityConfigBtn" disabled>${_t("save")}</button>
          </div>
        </div>

        <div class="config-section-header">
          <h3 class="config-section-title">${_t("block_rules")}</h3>
        ...
        <div class="danger-zone">
          <h3 class="config-section-title danger">${_t("danger_zone")}</h3>
  ...
  document.getElementById("copyKeyBtn").addEventListener("click", async () => {
    try {
      ...
      btn.innerHTML =
        '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg> ' + _t("copied") + '';
      setTimeout(() => {
        btn.innerHTML =
          '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg> ' + _t("copy_site_key") + '';
      }, 2000);
    } catch {
      showModal(
        _t("site_key_dialog"),
        `<div class="modal-body"><div class="modal-field"><label>${_t("site_key_label")}</label><input type="text" value="${key.siteKey}" readonly onclick="this.select()"></div></div>`,
      );
    }
  });
...
function updateGeoSource() {
  hasGeoSource = !!(
    headerSettings?.countryHeader ||
    (typeof ipdbStatus !== "undefined" && ipdbStatus?.mode && ipdbStatus.mode !== "")
  );
}
...
function renderChart(chartData) {
  ...
  chart = new Chart(ctx, {
    ...
    data: {
      labels,
      datasets: [
        mkDataset(_t("chart_challenges"), "challenges", "#89b4fa"),
        mkDataset(_t("chart_verified"), "verified", "#a6e3a1"),
        mkDataset(_t("chart_failed"), "failed", "#f38ba8"),
      ],
    },
    options: {
      responsive: true,
...
function setupMapTooltip(canvas) {
  ...
  canvas.addEventListener("mousemove", (e) => {
    ...
    if (found) {
      ...
      tooltip.innerHTML = `${flag} <b>${escapeHtml(name)}</b>${count > 0 ? ` \u2022 ${formatCompact(count)} ` + _t("challenges") : ""}`;
...
async function loadGeoStats() {
  ...
  if (netEl) {
    if (!data.asns || data.asns.length === 0) {
      netEl.innerHTML = `<div class="insight-empty">${hasGeoSource ? _t("no_network_has_source") : _t("no_network_no_source")}</div>`;
    } else {
...
function renderPlatformDonut(el, items, total) {
  ...
  if (!items || items.length === 0) {
    el.innerHTML = '<div class="insight-empty">' + _t("no_platform_data") + '</div>';
    return;
  }
...
function renderOsPanel(el, items, total) {
  if (!el) return;
  if (!items || items.length === 0) {
    el.innerHTML = '<div class="insight-empty">' + _t("no_os_data") + '</div>';
    return;
  }
  const t = total || 1;
  const entriesEl = document.createElement("div");
...
function renderLocationPanel(el, countries, total) {
  if (!countries || countries.length === 0) {
    el.innerHTML = `<div class="insight-empty">${hasGeoSource ? _t("no_location_has_source") : _t("no_location_no_source")}</div>`;
    return;
  }
  ...
  if (locationMapMode) {
    ...
    if (canvas) {
      loadWorldMap()
        ...
        .catch(() => {
          el.innerHTML = '<div class="insight-empty">' + _t("failed_load_map") + '</div>';
        });
    }
  } else {
    const t = total || 1;
...
async function loadBlockedIps() {
  ...
  if (!Array.isArray(data) || data.length === 0) {
    container.innerHTML = '<div class="blocked-ips-empty">' + _t("no_block_rules") + '</div>';
    return;
  }
  ...
  container.innerHTML = data
    .map((b) => {
      ...
      <div class="blocked-ip-info">
        ...
        <span class="blocked-ip-meta">${b.permanent ? _t("permanent") : _t("expires_date", formatDate(b.expires))}</span>
      </div>
      <button class="blocked-ip-unblock" data-type="${type}" data-value="${escapeHtml(b.ip)}">${_t("remove_rule")}</button>
    </div>`;
    })
    .join("");
...
function openAddBlockRuleModal() {
    ...
    <div class="modal-body">
      <div class="modal-field">
        ...
        <select id="blockRuleType">
          <option value="ip">${_t("ip_address")}</option>
          <option value="cidr">${_t("ip_range")}</option>
          <option value="asn">${_t("asn_option")}</option>
          <option value="country">${_t("country_option")}</option>
        </select>
      </div>
      <div class="modal-field" id="blockRuleValueField">
        <label id="blockRuleValueLabel">${_t("ip_address")}</label>
        <input type="text" id="blockRuleValue" placeholder="${_t("ip_placeholder")}">
      </div>
      <div class="modal-field" id="blockRuleCountryField" style="display:none">
        <label>${_t("country_code")}</label>
        <select id="blockRuleCountry">
          <option value="">${_t("select_country")}</option>
      ...
      <div class="modal-field">
        ...
        <select id="blockRuleDuration">
          <option value="0">${_t("perm_option")}</option>
          <option value="3600">${_t("hour_1")}</option>
          <option value="86400">${_t("hours_24")}</option>
          <option value="604800">${_t("days_7")}</option>
          <option value="2592000">${_t("days_30")}</option>
        </select>
      </div>
    </div>
    <div class="modal-footer">
      <button class="modal-btn secondary" id="blockRuleCancelBtn">${_t("cancel")}</button>
      <button class="modal-btn danger" id="blockRuleSubmitBtn">${_t("block")}</button>
    </div>`,
  );
  ...
  const placeholders = { ip: _t("ip_placeholder"), cidr: _t("cidr_placeholder"), asn: _t("asn_placeholder") };
  const labels = { ip: _t("ip_value"), cidr: _t("ip_range_value"), asn: _t("asn_value") };
...
async function saveMainConfig() {
  ...
  if (!name || difficulty < 1 || challengeCount < 1) {
    showModal(
      _t("error"),
      '<div class="modal-body"><p>' + _t("check_input") + '</p></div>',
    );
    btn.disabled = false;
    return;
  }
  ...
  if (rsw) {
    btn.innerHTML = _t("preparing_rsw");
    const gen = await api("POST", "/settings/rsw/ensure");
    if (!gen?.exists) {
      showModal(
        _t("error"),
        '<div class="modal-body"><p>' + _t("failed_generate_rsw") + '</p></div>',
      );
      btn.innerHTML = _t("save");
      btn.disabled = false;
      return;
    }
    btn.innerHTML = "Save";
  ...
  if (res.success) {
    ...
    showModal(_t("error"), '<div class="modal-body"><p>' + _t("failed_save_config") + '</p></div>');
    btn.disabled = false;
  }
}
...
async function saveSecurityConfig() {
  ...
  if (res.success) {
    ...
    showModal(_t("error"), '<div class="modal-body"><p>' + _t("failed_save_security") + '</p></div>');
    btn.disabled = false;
  }
}
  ...
  showConfirmModal(
    ...
    async () => {
      ...
      if (res.secretKey) {
        showModal(
          _t("rotated_title"),
          `<div class="modal-body"><div class="modal-field"><label>${_t("new_secret_key")}</label>
          <input type="text" value="${res.secretKey}" readonly onclick="this.select()">
          <p class="hint">${_t("copy_new_secret_hint")}</p></div></div>`,
        );
      } else {
        showModal(_t("error"), '<div class="modal-body"><p>' + _t("failed_rotate_secret") + '</p></div>');
      }
    },
  );
}
  ...
  showConfirmModal(
    ...
    async () => {
      ...
      if (res.success) {
        ...
        showModal(_t("error"), '<div class="modal-body"><p>' + _t("failed_delete_key") + '</p></div>');
      }
    },
    true,
  );
...
function showConfirmModal(title, message, confirmText, onConfirm, isDanger = false) {
  const modal = createModal(
    ...
    <div class="modal-footer">
      <button class="modal-btn secondary" id="cancelBtn">${_t("cancel")}</button>
...
function openCreateKeyModal(prefill = "") {
  const modal = createModal(
    ...
    `
    <div class="modal-body">
      <div class="modal-field"><label for="newKeyName">${_t("key_name")}</label>
      ...
      <div class="switch-field" style="margin-top:8px">
        ...
        <label for="newKeyInstrumentation" class="switch-label">${_t("enable_instrumentation")}</label>
      </div>
      <div class="switch-field" id="newKeyBlockBotsField" style="display:none">
        ...
        <label for="newKeyBlockBots" class="switch-label">${_t("block_headless")}</label>
      </div>
      <div class="switch-field">
        <label class="switch">
        ...
        <label for="newKeyCorsEnabled" class="switch-label">
          ${_t("restrict_origins_label")}
          <span class="hint">${_t("origins_hint")}</span>
        </label>
      </div>
      <div id="newKeyCorsPanel" style="display:none">
        <div id="newKeyCorsOriginsList" class="origin-list">
          <div class="origin-entry"><input type="text" class="new-key-cors-origin-input" placeholder="${_t("origin_placeholder")}"><button class="origin-remove-btn" title="${_t("remove")}">&times;</button></div>
        </div>
      </div>
    </div>
    <div class="modal-footer">
      <button class="modal-btn secondary" onclick="closeModal()">${_t("cancel")}</button>
      <button class="modal-btn primary" id="createKeySubmit">${_t("create")}</button>
    </div>`,
  );
  ...
  submitBtn.addEventListener("click", async () => {
    ...
    if (res.siteKey && res.secretKey) {
      closeModal();
      showModal(
        _t("key_created_title"),
        `
        <div class="modal-body">
          <div class="modal-field"><label>${_t("site_key_label")}</label><input type="text" value="${res.siteKey}" readonly onclick="this.select()"></div>
          <div class="modal-field"><label>${_t("secret_key_label")}</label><input type="text" value="${res.secretKey}" readonly onclick="this.select()">
          <p class="hint">${_t("copy_secret_hint")}</p></div>
        </div>
        <div class="modal-footer">
          <button class="modal-btn secondary" onclick="closeModal()">${_t("close")}</button>
          <button class="modal-btn primary" onclick="closeModal(); selectKey('${res.siteKey}')">${_t("open_key")}</button>
        </div>`,
      );
      await loadKeys();
      selectKey(res.siteKey);
    } else {
      showModal(_t("error"), '<div class="modal-body"><p>' + _t("failed_create_key") + '</p></div>');
    }
  });
}
...
async function openSettings() {
  ...
  const modal = createModal(
    ...
    <div class="settings-tabs">
      ...
      <button class="settings-tab active" data-tab="sessions">${_t("settings_sessions")}</button>
      <button class="settings-tab" data-tab="security">${_t("settings_security")}</button>
      <button class="settings-tab" data-tab="ipdata">${_t("settings_ip_data")}</button>
      <button class="settings-tab" data-tab="apikeys">${_t("settings_api_keys")}</button>
      <button class="settings-tab" data-tab="about">${_t("settings_about")}</button>
    </div>
    <div class="settings-content">
      ...
      <div class="settings-section" id="securitySection">
        <h4 class="settings-group-title">${_t("global_rate_limit")}</h4>
        <p class="headers-description">${_t("global_rate_desc")}</p>
        <div class="edit-row">
          <div class="edit-field">
            <label>${_t("max_requests")}</label>
            <input type="number" id="cfgGlobalRatelimitMax" value="${rl.max}" min="1" max="10000">
            <span class="field-hint">${_t("requests_per_window")}</span>
          </div>
          <div class="edit-field">
            <label>${_t("window_ms")}</label>
            <input type="number" id="cfgGlobalRatelimitDuration" value="${rl.duration}" min="1000" max="3600000" step="1000">
            <span class="field-hint">${_t("window_hint")}</span>
          </div>
        </div>
        <div class="config-save-row">
          <button class="save-btn" id="saveGlobalRatelimitBtn" disabled>${_t("save")}</button>
        </div>
        ...
        <div class="switch-field">
          ...
          <label for="cfgGlobalCorsEnabled" class="switch-label">${_t("restrict_origins")}</label>
        </div>
        <div id="globalCorsPanel" style="display:${corsOriginsList.length ? "block" : "none"}">
          <p class="headers-description" style="margin:0 0 8px">${_t("global_origins_hint")}</p>
          <div id="globalCorsOrigins" class="origin-list">
            ${corsOriginsList.map((o) => `<div class="origin-entry"><input type="text" class="cors-origin-input" value="${escapeHtml(o)}" placeholder="${_t("add_origin")}"><button class="origin-remove-btn" title="${_t("remove")}">&times;</button></div>`).join("")}
          </div>
        </div>
        <div class="config-save-row">
          <button class="save-btn" id="saveCorsBtn" disabled>${_t("save")}</button>
        </div>

        <hr class="settings-divider">

        <h4 class="settings-group-title">${_t("request_filtering")}</h4>
        <p class="headers-description">${_t("global_filtering_desc")}</p>
        <div class="switch-field">
          <label class="switch">
        ...
        <div class="config-save-row">
          <button class="save-btn" id="saveFilteringBtn" disabled>${_t("save")}</button>
        </div>
      </div>
      <div class="settings-section" id="ipdataSection">
        <h4 class="settings-group-title">${_t("ip_header")}</h4>
        <p class="headers-description">${_t("ip_header_desc")}</p>
        <div class="headers-presets">
          <span class="headers-presets-label">${_t("presets")}</span>
          <button class="preset-btn" data-preset="cloudflare">${_t("cloudflare")}</button>
          <button class="preset-btn" data-preset="vercel">${_t("vercel")}</button>
          <button class="preset-btn" data-preset="nginx">${_t("nginx")}</button>
          <button class="preset-btn" data-preset="clear">${_t("clear")}</button>
        </div>
        <div class="edit-field">
          <label>${_t("ip_header_label")}</label>
          <input type="text" id="cfgIpHeader" placeholder="${_t("ip_header_placeholder")}" value="${escapeHtml(hs.ipHeader || "")}">
        </div>
        <div class="config-save-row">
          <button class="save-btn" id="saveIpHeaderBtn" disabled>${_t("save")}</button>
        </div>

        <hr class="settings-divider">

        <h4 class="settings-group-title">${_t("country_asn")}</h4>
        <p class="headers-description">${_t("country_asn_desc")}</p>
        <div class="net-source-tabs">
          <button class="net-source-tab active" data-source="ipdb">${_t("tab_ip_database")}</button>
          <button class="net-source-tab" data-source="headers">${_t("tab_proxy_headers")}</button>
        </div>
        ...
        <div class="net-source-panel" id="netSourceHeaders">
          <div class="edit-field">
            <label>${_t("country_header")}</label>
            <input type="text" id="cfgCountryHeader" placeholder="${_t("country_header_placeholder")}" value="${escapeHtml(hs.countryHeader || "")}">
            <span class="field-hint">${_t("country_header_hint")}</span>
          </div>
          <div class="edit-field">
            <label>${_t("asn_header")}</label>
            <input type="text" id="cfgAsnHeader" placeholder="${_t("asn_header_placeholder")}" value="${escapeHtml(hs.asnHeader || "")}">
            <span class="field-hint">${_t("asn_header_hint")}</span>
          </div>
          <div class="config-save-row">
            <button class="save-btn" id="saveGeoHeadersBtn" disabled>${_t("save")}</button>
          </div>
        </div>
      </div>
      <div class="settings-section" id="apikeysSection">
        <div style="display:flex;justify-content:flex-end;margin-bottom:12px">
          <button class="create-btn" id="createApiKeyBtn">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M12 5v14M5 12h14"/></svg> ${_t("create")}
          </button>
        </div>
      ...
      <div class="session-item">
        <div class="session-info">
          ...
          <div class="session-meta">
            ${currentHash.endsWith(s.token) ? '<span class="current">' + _t("current") + '</span> \u2022 ' : ""}
            ${_t("expires", formatRelative(s.expires))}
          </div>
        </div>
        <button class="session-action" data-token="${s.token}">${_t("logout")}</button>
      </div>`,
      )
      .join("");
  ...
  if (Array.isArray(apikeys)) {
    if (apikeys.length === 0) {
      document.getElementById("apikeysList").innerHTML =
        '<div class="empty-list">' + _t("no_api_keys") + '</div>';
    } else {
      document.getElementById("apikeysList").innerHTML = apikeys
        ...
        <div class="apikey-item">
          <div class="apikey-info">
            ...
            <div class="apikey-meta">${_t("key_id_meta", k.id.slice(0, 12), formatRelative(k.created))}</div>
          </div>
          <button class="apikey-action" data-id="${k.id}">${_t("delete")}</button>
        </div>`,
        )
        .join("");
      ...
      document.querySelectorAll(".apikey-action").forEach((btn) => {
        btn.addEventListener("click", () => {
          showConfirmModal(
            _t("delete_apikey_title"),
            _t("delete_apikey_msg"),
            _t("delete"),
            async () => {
  ...
  if (about.ver) {
    document.getElementById("aboutInfo").innerHTML =
      `${_t("version_info", escapeHtml(about.ver))}${about.demo ? '<br><span style="color:var(--blue);font-weight:500">' + _t("demo_mode_badge") + '</span>' : ""}`;
  }
}

function openCreateApiKeyModal() {
  const modal = createModal(
    _t("create_apikey_title"),
    `
    <div class="modal-body">
      <div class="modal-field"><label for="apiKeyName">${_t("apikey_name")}</label>
      <input type="text" id="apiKeyName"></div>
    </div>
    <div class="modal-footer">
      <button class="modal-btn secondary" onclick="closeModal(); openSettings()">${_t("cancel")}</button>
      <button class="modal-btn primary" id="createApiKeySubmit">${_t("create")}</button>
    </div>`,
  );
  ...
  submitBtn.addEventListener("click", async () => {
    ...
    if (res.apiKey) {
      closeModal();
      showModal(
        _t("apikey_created_title"),
        `<div class="modal-body"><div class="modal-field"><label>${_t("apikey_label")}</label>
        <input type="text" value="${res.apiKey}" readonly onclick="this.select()">
        <p class="hint">${_t("apikey_hint")}</p></div></div>
        <div class="modal-footer"><button class="modal-btn primary" onclick="closeModal(); openSettings()">${_t("done")}</button></div>`,
      );
    } else {
       showModal(_t("error"), '<div class="modal-body"><p>' + _t("failed_create_apikey") + '</p></div>');
    }
  });
}
...
async function loadIPDBSettings() {
  ...
  container.innerHTML = `
    ...
    ${
      ...
      <div class="ipdb-status-card">
        <div class="ipdb-status-row">
          ...
          <span class="ipdb-status-label">${data.mode === "dbip" ? _t("dbip_name") : data.mode === "maxmind" ? _t("maxmind_name") : _t("ipinfo_name")}</span>
          ${data.lastUpdated ? `<span class="ipdb-status-meta">${_t("updated_relative", formatRelative(data.lastUpdated))}</span>` : ""}
        </div>
        ${
          hasDB
            ? `<div class="ipdb-status-detail">
          ${data.country?.exists ? `<span>${_t("country_size", formatSize(data.country.size))}</span>` : ""}
          ${data.asn?.exists ? `<span>${_t("asn_size", formatSize(data.asn.size))}</span>` : ""}
        </div>`
            : ""
        }
        <div class="ipdb-actions">
          <button class="preset-btn" id="ipdbUpdateBtn">${_t("update")}</button>
          <button class="preset-btn" id="ipdbDeleteBtn" style="color:var(--red)">${_t("delete_db")}</button>
        </div>
      </div>
    `
        : `
      <div class="ipdb-setup">
        <div class="edit-field">
          <label>${_t("provider")}</label>
          <select id="ipdbMode">
            <option value="dbip">${_t("dbip_lite")}</option>
            <option value="maxmind">${_t("maxmind")}</option>
            <option value="ipinfo">${_t("ipinfo")}</option>
          </select>
        </div>
        <div class="edit-field" id="ipdbMaxmindAccountField" style="display:none">
          <label>${_t("maxmind_account")}</label>
          <input type="text" id="ipdbMaxmindAccountId" placeholder="${_t("maxmind_account_placeholder")}">
        </div>
        <div class="edit-field" id="ipdbMaxmindField" style="display:none">
          <label>${_t("maxmind_license")}</label>
          <input type="text" id="ipdbMaxmindKey" placeholder="${_t("maxmind_license_placeholder")}">
        </div>
        <div class="edit-field" id="ipdbIpinfoField" style="display:none">
          <label>${_t("ipinfo_token")}</label>
          <input type="text" id="ipdbIpinfoToken" placeholder="${_t("ipinfo_token_placeholder")}">
        </div>
        <button class="save-btn" id="ipdbDownloadBtn">${_t("download_activate")}</button>
      </div>
    `
    }
    <div class="ipdb-progress" id="ipdbProgress" style="display:none">
      ...
      <span class="ipdb-progress-text" id="ipdbProgressText">${_t("downloading")}</span>
    </div>
  `;
  ...
  if (!isActive) {
    ...
    container.querySelector("#ipdbDownloadBtn")?.addEventListener("click", async () => {
      ...
      downloadBtn.innerHTML = `${IPDB_SPINNER_SVG} ${_t("starting_download")}`;
      const res = await api("POST", "/settings/ipdb/download", body);
      if (!res.success) {
        showModal(
          _t("error"),
          `<div class="modal-body"><p>${escapeHtml(res.error || _t("download_failed"))}</p></div>`,
        );
        downloadBtn.disabled = false;
        downloadBtn.innerHTML = _t("download_activate");
        return;
      }

      downloadBtn.innerHTML = `${IPDB_SPINNER_SVG} ${_t("downloading")}`;
      ...
      const pollFn = async () => {
        ...
        if (p.active) {
          ...
          if (p.total > 0) {
            ...
            label = _t("downloading_file", p.file, pct);
          } else {
            if (fill) {
            ...
            label = _t("downloading_kb", p.file, kb);
          }
    ...
    container.querySelector("#ipdbUpdateBtn")?.addEventListener("click", async () => {
      ...
      updateBtn.innerHTML = `${IPDB_SPINNER_SVG} ${_t("updating")}`;
