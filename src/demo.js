const DEMO_KEYS = [
  { siteKey: "a1b2c3d4e5", name: "production-web", created: Date.now() - 90 * 86400000, traffic: 2_800_000, failRate: 0.06, config: { difficulty: 4, challengeCount: 80, saltSize: 32, instrumentation: true, obfuscationLevel: 5, blockAutomatedBrowsers: true, ratelimitMax: null, ratelimitDuration: null, corsOrigins: ["example.com", "www.example.com"], blockNonBrowserUA: true, requiredHeaders: ["accept-language", "sec-ch-ua"] } },
  { siteKey: "f6g7h8i9j0", name: "mobile-app", created: Date.now() - 75 * 86400000, traffic: 420_000, failRate: 0.04, config: { difficulty: 3, challengeCount: 50, saltSize: 32, instrumentation: true, obfuscationLevel: 3, blockAutomatedBrowsers: false, ratelimitMax: 50, ratelimitDuration: 10000, corsOrigins: null, blockNonBrowserUA: false, requiredHeaders: null } },
  { siteKey: "k1l2m3n4o5", name: "checkout-flow", created: Date.now() - 60 * 86400000, traffic: 890_000, failRate: 0.09, config: { difficulty: 5, challengeCount: 120, saltSize: 32, instrumentation: true, obfuscationLevel: 7, blockAutomatedBrowsers: true, ratelimitMax: 20, ratelimitDuration: 5000, corsOrigins: ["shop.example.com"], blockNonBrowserUA: true, requiredHeaders: ["accept-language", "sec-ch-ua", "sec-ch-ua-platform"] } },
  { siteKey: "p6q7r8s9t0", name: "api-gateway", created: Date.now() - 45 * 86400000, traffic: 3_500_000, failRate: 0.14, config: { difficulty: 4, challengeCount: 80, saltSize: 32, instrumentation: true, obfuscationLevel: 4, blockAutomatedBrowsers: true, ratelimitMax: 100, ratelimitDuration: 5000, corsOrigins: null, blockNonBrowserUA: false, requiredHeaders: null } },
  { siteKey: "u1v2w3x4y5", name: "staging", created: Date.now() - 30 * 86400000, traffic: 1_200, failRate: 0.02, config: { difficulty: 2, challengeCount: 30, saltSize: 32, instrumentation: false, obfuscationLevel: 1, blockAutomatedBrowsers: false, ratelimitMax: null, ratelimitDuration: null, corsOrigins: null, blockNonBrowserUA: false, requiredHeaders: null } },
  { siteKey: "z6a7b8c9d0", name: "partner-portal", created: Date.now() - 55 * 86400000, traffic: 165_000, failRate: 0.07, config: { difficulty: 4, challengeCount: 80, saltSize: 32, instrumentation: true, obfuscationLevel: 5, blockAutomatedBrowsers: false, ratelimitMax: null, ratelimitDuration: null, corsOrigins: ["partners.example.com"], blockNonBrowserUA: true, requiredHeaders: ["accept-language"] } },
  { siteKey: "e1f2g3h4i5", name: "marketing-site", created: Date.now() - 20 * 86400000, traffic: 310_000, failRate: 0.05, config: { difficulty: 3, challengeCount: 60, saltSize: 32, instrumentation: true, obfuscationLevel: 3, blockAutomatedBrowsers: false, ratelimitMax: null, ratelimitDuration: null, corsOrigins: null, blockNonBrowserUA: false, requiredHeaders: null } },
];
...
const COUNTRY_WEIGHTS = [
  ["US", 28], ["GB", 9], ["DE", 7], ["PT", 6], ["FR", 5], ["CA", 4], ["AU", 3.5], ["NL", 3], ["BR", 2.8], ["IN", 2.5],
  ["JP", 2.2], ["KR", 1.8], ["ES", 1.7], ["IT", 1.5], ["PL", 1.4], ["SE", 1.2], ["MX", 1.1], ["RU", 1.0], ["ID", 0.9],
  ["TR", 0.8], ["CH", 0.7], ["AT", 0.6], ["BE", 0.6], ["NO", 0.5], ["DK", 0.5], ["FI", 0.5], ["PT", 0.5], ["IE", 0.5],
  ["CZ", 0.4], ["RO", 0.4], ["SG", 0.4], ["IL", 0.4], ["NZ", 0.3], ["TH", 0.3], ["PH", 0.3], ["UA", 0.3], ["AR", 0.3],
  ["CO", 0.2], ["ZA", 0.2], ["HK", 0.2], ["TW", 0.2], ["CL", 0.2], ["AE", 0.2], ["VN", 0.2], ["MY", 0.2], ["NG", 0.15],
  ["EG", 0.1], ["SA", 0.1], ["PK", 0.1],
];
...
function generateChartData(rng, key, duration) {
  ...
  switch (duration) {
    ...
    case "alltime":
      bucketSize = day;
      numBuckets = Math.ceil((Date.now() - key.created) / 86400000);
      startTime = Math.floor(now / day) * day - (numBuckets - 1) * day;
      endTime = Math.floor(now / day) * day + day;
      break;
    default:
      bucketSize = hour;
  ...
  const data = [];
  let totalChallenges = 0, totalVerified = 0, totalFailed = 0, totalLatSum = 0, totalLatCount = 0, totalRateLimited = 0;
  ...
  for (let i = 0; i < numBuckets; i++) {
    ...
    if (challenges > 0) { totalLatSum += avgLatency * challenges; totalLatCount += challenges; }
    data.push({ bucket, challenges, verified, failed, rateLimited });
  }

  return {
    stats: { challenges: totalChallenges, verified: totalVerified, failed: totalFailed, rateLimited: totalRateLimited, avgLatency: totalLatCount > 0 ? Math.round(totalLatSum / totalLatCount) : 0 },
    chartData: { duration, bucketSize, data },
  };
}

function generateBlockedIps(rng) {
  const BLOCKED_RULES = [
  ...
  for (let i = 0; i < numRules; i++) {
    let idx; do { idx = Math.floor(rng() * BLOCKED_RULES.length); } while (used.has(idx));
    used.add(idx);
    const rule = BLOCKED_RULES[idx];
    rules.push({ ip: rule.value, type: rule.type, permanent: rule.permanent, expires: rule.permanent ? null : Date.now() + rule.expiresIn });
  }
  return rules;
}
...
function generateWeightedStats(rng, total, weights) {
  ...
  return weights.map(([name, weight]) => {
    ...
    const jitter = 1 + (rng() - 0.5) * 0.4;
    return { name, count: Math.max(1, Math.round(base * jitter)) };
  }).sort((a, b) => b.count - a.count);
}
...
export function demoGetKey(siteKey, chartDuration = "today") {
  ...
  return { key: { siteKey: key.siteKey, name: key.name, created: key.created, config: { ...key.config } }, stats, prevStats, chartData };
}
...
export function demoGetGeoStats(siteKey) {
  ...
  const asns = generateASNStats(mulberry32(hashStr(`${siteKey}asn`)), key.traffic);
  const platforms = generateWeightedStats(mulberry32(hashStr(`${siteKey}platform`)), key.traffic, PLATFORM_WEIGHTS);
  const oses = generateWeightedStats(mulberry32(hashStr(`${siteKey}os`)), key.traffic, OS_WEIGHTS);
  return {
    countries, totalCountry: countries.reduce((s, c) => s + c.count, 0),
    asns, totalAsn: asns.reduce((s, a) => s + a.count, 0),
    platforms, totalPlatform: platforms.reduce((s, p) => s + p.count, 0),
    oses, totalOs: oses.reduce((s, o) => s + o.count, 0),
  };
}

export function demoGetBlockedIps(siteKey) {
  return generateBlockedIps(mulberry32(hashStr(`${siteKey}blocked`)));
}
