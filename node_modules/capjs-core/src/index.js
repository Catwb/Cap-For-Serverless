...
// Cache the minter on the keypair object identity, keyed by t. Callers who
// pass the same `keypair` object back across requests get zero per-call
// overhead beyond the mint itself.
const _minterCache = new WeakMap(); // keypair → Map<t, minter>

function resolveRswMinter(opts) {
  // explicit minter takes precedence (existing API)
  ...
  // Normalize: serialized (strings) → BigInt
  const kp = typeof opts.keypair.N === "string"
    ? deserializeRswKeypair(opts.keypair)
    : opts.keypair;

  // Memoize {keypair, t} → minter. Reuses the precomputed h across calls so
  // the first request after boot is the only one that pays the ~5 ms setup.
